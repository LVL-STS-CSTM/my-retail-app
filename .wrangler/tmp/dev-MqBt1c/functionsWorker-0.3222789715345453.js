...
// .wrangler/tmp/pages-gtXiAl/functionsWorker-0.3222789715345453.mjs
...
var onRequestPost = /* @__PURE__ */ __name2(async (context22) => {
  ...
  try {
    ...
    const storedCredsRaw = await env22.CONTENT_KV.get("credential");
    if (!storedCredsRaw) {
...
var onRequestGet = /* @__PURE__ */ __name2(async (context22) => {
  ...
  try {
    for (const [key, data] of Object.entries(DATA_TO_SEED)) {
      await env22.CONTENT_KV.put(key, JSON.stringify(data));
    }
...
var getAuthToken = /* @__PURE__ */ __name2((req) => {
...
var isAuthenticated = /* @__PURE__ */ __name2(async (token, env22) => {
...
var onRequestGet2 = /* @__PURE__ */ __name2(async (context22) => {
  const key = context22.params.key;
  const data = await context22.env.CONTENT_KV.get(key, "json");
  if (data === null) {
...
var onRequestPost2 = /* @__PURE__ */ __name2(async (context22) => {
  const key = context22.params.key;
  const token = getAuthToken(context22.request);
  if (!await isAuthenticated(token, context22.env)) {
    return new Response("Unauthorized", { status: 401 });
  }
  try {
    const body = await context22.request.json();
    await context22.env.CONTENT_KV.put(key, JSON.stringify(body));
    return new Response(JSON.stringify({ success: true, message: `Data for key '${key}' updated.` }), {
      status: 200,
      headers: { "Content-Type": "application/json" }
    });
  } catch (error32) {
    return new Response(JSON.stringify({ message: "Error saving data", error: error32.message }), {
      status: 500,
      headers: { "Content-Type": "application/json" }
    });
  }
...
${JSON.stringify(products, null, 2)}`
    ...
    if (error32.name === "AbortError") {
      console.error("Gemini request timed out");
      return new Response(JSON.stringify({ error: "The request timed out. Please try again." }), { status: 504 });
    }
    console.error("Error processing Gemini request:", error32);
    return new Response(JSON.stringify({ error: "An internal server error occurred.", details: error32.message }), { status: 500 });
  }
}, "onRequestPost");
...
async function getAccessToken(env22) {
  const jwt = await signJwt(env22.GOOGLE_SERVICE_ACCOUNT_EMAIL, env22.GOOGLE_PRIVATE_KEY, "https://www.googleapis.com/auth/spreadsheets");
...
var onRequestPost4 = /* @__PURE__ */ __name2(async (context22) => {
  ...
  try {
    ...
    const token = await getAccessToken(env22);
    ...
    const sheetRes = await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${env22.GOOGLE_SHEET_ID}/values/Quotes!A1:append?valueInputOption=USER_ENTERED`,
      {
        method: "POST",
...
async function getAccessToken2(env22) {
  const jwt = await signJwt2(env22.GOOGLE_SERVICE_ACCOUNT_EMAIL, env22.GOOGLE_PRIVATE_KEY, "https://www.googleapis.com/auth/spreadsheets");
  const res = await fetch("https://oauth2.googleapis.com/token", {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
    body: `grant_type=urn:ietf:params:oauth:grant-type:jwt-bearer&assertion=${jwt}`
...
var onRequestPost5 = /* @__PURE__ */ __name2(async (context22) => {
  ...
  try {
    ...
    const token = await getAccessToken2(env22);
    const body = {
      values: [[(/* @__PURE__ */ new Date()).toISOString(), email.toLowerCase().trim()]]
    };
    const sheetRes = await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${env22.GOOGLE_SHEET_ID}/values/Subscriptions!A1:append?valueInputOption=USER_ENTERED`,
      {
        method: "POST",
        headers: { "Authorization": `Bearer ${token}`, "Content-Type": "application/json" },
        body: JSON.stringify(body)
...
// .wrangler/tmp/bundle-7UCGYA/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-7UCGYA/middleware-loader.entry.ts
...
@google/generative-ai/dist/index.mjs:
   ...
   *)
*/
//# sourceMappingURL=functionsWorker-0.3222789715345453.js.map
